...
**어떻게 만들었나**
[파일 구조](#파일-구조) ·
[두 클래스가 각각 맡는 일](#두-클래스가-각각-맡는-일) ·
[호출 흐름](#호출-흐름) ·
[왜 함수 말고 클래스로 했나](#왜-함수-말고-클래스로-했나)

**데이터를 어떻게 다루나**
[데이터 파일 설명](#데이터-파일-설명-statejson) ·
[왜 JSON 으로 저장했나](#왜-json-으로-저장했나) ·
[저장 파일이 깨지지 않게 하는 방법](#저장-파일이-깨지지-않게-하는-방법) ·
[필드 구조](#필드-구조)

**더 생각해본 것**
[퀴즈가 많아지면](#퀴즈가-많아지면) ·
[뭘 바꾸고 싶을 때 어디를 고치면 되나](#뭘-바꾸고-싶을-때-어디를-고치면-되나)
...
터미널에서 돌아가는 모습이에요. 아래는 실제로 돌려서 나온 출력이에요.

### 메뉴

실행하면 저장된 내용을 먼저 불러오고 메뉴를 띄워요.

![메뉴 화면](../docs/screenshots/menu.png)

### 퀴즈 추가

문제랑 선택지 4개, 정답 번호를 차례로 받아서 등록하고 바로 저장해요.

![퀴즈 추가](../docs/screenshots/add_quiz.png)

### 퀴즈 목록

등록된 문항을 번호와 함께 보여줘요. 방금 추가한 문제가 마지막에 붙어요.

![퀴즈 목록](../docs/screenshots/list.png)

### 퀴즈 풀기

한 문제씩 내고 바로 정답인지 알려줘요. 다 풀면 맞힌 개수와 정답률이 나와요.
...
숫자가 아닌 값, 범위 밖 숫자, 빈 입력을 각각 걸러내고 다시 물어봐요.
