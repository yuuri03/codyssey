...
| About | 프로필 이미지와 자기소개 (뮤지컬 전공에서 개발로 옮겨 온 이야기) |
| Skills | 팀 프로젝트에서 실제로 써 본 기술만 카드로 정리 |
| Projects | 팀 프로젝트 2건(정적) + GitHub API 로 불러온 개인 저장소(동적) |
| Contact | 연락처와 문의 폼 |
| Footer | 저작권과 소셜 링크 |

### 디자인

색과 서체는 따로 만들지 않고, 인쇄용으로 쓰던 포트폴리오의 것을 그대로 옮겼습니다.
아이보리 바탕(`#faf8ef`)에 테라코타 레드(`#a5311a`), 제목은 세리프, 표지에 쓰던
타원 배지는 Hero 의 인사말 자리에 두었습니다. 같은 사람의 문서라는 것이 한눈에
보이는 편이 좋다고 판단했습니다. 색을 CSS 변수로 모아 둔 덕분에 값만 갈아 끼우면
되었고, 레이아웃 코드는 한 줄도 건드리지 않았습니다.
...
Projects 섹션은 두 묶음으로 나뉩니다. 위쪽 **주요 프로젝트**는 팀으로 만든 서비스
두 건을 담은 정적 카드이고, 아래쪽 **GitHub 저장소**는 API 로 그때그때 불러오는
동적 카드입니다. 아래 상태 이야기는 뒤쪽에만 해당합니다.

요청 결과에 따라 네 화면 중 하나만 보여 줍니다.
...
| `article` | Skills 카드, 주요 프로젝트 카드, GitHub 저장소 카드 | 떼어 놓아도 그 자체로 말이 되는 조각 |
...
이 이점은 만드는 중에 실제로 확인했습니다. 처음에는 남색 계열로 만들었다가
인쇄용 포트폴리오와 같은 테라코타 레드로 바꿨는데, `:root` 와
`[data-theme="dark"]` 의 색 변수 열몇 줄만 고쳐서 끝났습니다. 버튼·링크·태그·
스피너·포커스 테두리처럼 그 색을 쓰던 자리를 찾아다닐 필요가 없었습니다.
